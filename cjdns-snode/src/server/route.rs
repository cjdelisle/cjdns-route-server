...
fn get_route_impl(nodes: &Nodes, routing: &mut Routing, src: Arc<Node>, dst: Arc<Node>) -> Result<Route, RoutingError> {
    let now = Instant::now();
...
fn compute_route(nodes: &Nodes, routing: &mut Routing, src: Arc<Node>, dst: Arc<Node>) -> Result<Route, RoutingError> {
...
fn compute_routing_label(nodes: &Nodes, rev_path: &[CJDNS_IP6]) -> Result<(RoutingLabel<u64>, Vec<Hop>), RoutingError> {
    let (labels, hops) = {
