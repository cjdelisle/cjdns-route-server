...
use crate::types::{AlloyEvent, AlloyFilledProvider, AlloyProviderWs, GenericProvider, MkProvider};
use crate::rpcinstance::{RpcInfo, RpcInstance};
...
struct MkNoWalletProvider();
impl MkProvider<AlloyFilledProvider> for MkNoWalletProvider {
    fn mk_provider(&self, url: Url) -> AlloyFilledProvider {
        ProviderBuilder::new()
...
impl EthRpc {
    pub async fn new(config: &EthRpcConfig) -> Result<Arc<Self>> {
        ...
        Ok(Arc::new(Self{
            ...
            rpcs,
            rpc_max_tries: config.rpc_max_tries.unwrap_or(5) as i32,
        }))
    }
            ...
            FY: IntoFuture<Output=eyre::Result<Y>>,
    {
        ...
        loop {
            i += 1;
            let (rpc, rpc_info) = self.rpc(MkNoWalletProvider()).await?;
    ...
    pub(crate) async fn rpc<P:GenericProvider>(&self, mk_provider: impl MkProvider<P>) -> Result<(RpcInstance<P>,Arc<RpcInfo>)> {
    ...
    async fn rpc_of_type<P:GenericProvider>(
        ...
        let provider: P = mk_provider.mk_provider(url);
        Ok((
            Some((
                RpcInstance::new(
                    &out,
